...
public class CartDAO {
	...
	public static ArrayList<Cart> selectByUserId(Long userId) {
		ArrayList<Cart> arr = new ArrayList<>();
		try {
			Connection conn = JDBCUtil.getConnection();
	...
	public static ArrayList<Cart> selectByUserIds(Long userId, java.util.List<Integer> productIds) {
		ArrayList<Cart> arr = new ArrayList<>();
		...
		try {
			...
			String sql = "SELECT * FROM carts WHERE user_id=? AND product_id IN (" + placeholders.toString() + ")";
			PreparedStatement st = conn.prepareStatement(sql);
			st.setLong(1, userId);

			for (int i = 0; i < productIds.size(); i++) {
				st.setLong(i + 2, productIds.get(i));
			}
	...
	public static int deleteByUserIdAndProductId(Long userId, Long productId) {
		int res = 0;
		try (Connection conn = JDBCUtil.getConnection()) {
			...
			System.out.println("Delete Cart SQL: " + sql + ", params: user_id=" + userId + ", product_id=" + productId);
			res = st.executeUpdate();
			System.out.println("Cart delete result: " + res);

			st.close();
		} catch (Exception e) {
			System.out.println("CartDAO delete error:");
			e.printStackTrace();
		}
		return res;
	}
